...
lapply(c("data.table", "lubridate", "zoo", "dplyr","readr"), require, character.only=TRUE)

# read the fwf data file
cases <- read_fwf("Probation_Case_20210311.txt", fwf_cols("submit_date"=c(1,10), "Agency"=c(11,19),"source"=c(20,22),"xnmbr"=c(23,31),"prob_stts_code"=c(32,33),"prim_chrg_code"=c(34,35),"pri_chrg_stt"=c(36,53),"priy_chrg_id"=c(54,56),"pri_chrg_deg"=c(57,57),"pri_chrg_lvl"=c(58,58),"crt_case_nmbr"=c(59,71),"case_stts_code"=c(72,73),"case_grant_date"=c(74,83),"sup_close_date"=c(84,93),"race"=c(94,94),"area_office_code"=c(95,100),"area_office_name"=c(101,120),"area_office_addr"=c(121,155),"area_office_city"=c(156,175),"area_office_state"=c(176,177),"area_office_zip"=c(178,186),"cnt_grant_type_ind"=c(187,207),"supvn_caseld_type"=c(208,210),"homeless_ind"=c(211,211),"dpo_lstname"=c(212,231),"dpo_1stname"=c(232,246),"dpo_nmbr"=c(247,256),"dpo_email"=c(257,306),"drug"=c(307,307),"sex"=c(308,308),"arson"=c(309,309),"cjis_charge_code"=c(310,311),"record_type"=c(312,312)))

# extract month and year from case grant date and sup closing date from case files 
cases1$month_grant <- month(as.POSIXlt(cases1$case_grant_date, format="%m/%d/%Y"))
cases1$year_grant <-  year(as.POSIXlt(cases1$case_grant_date, format="%m/%d/%Y"))
cases1$month_closing <- month(as.POSIXlt(cases1$sup_closing_date, format="%m/%d/%Y"))
cases1$year_closing <-  year(as.POSIXlt(cases1$sup_closing_date, format="%m/%d/%Y"))
...
# individual growth curves (takes time to generate)
...
###########################################
## A whole new approach inspired by the
## BOS study
###########################################

# Split sentencing individual by year
setDT(cases)[year_grant == 2015 & cnt_grant_type_ind == 'MANDATORY SUPERVISION',.N, by=xnmbr]
cases1[year_grant == 2012 & cnt_grant_typ_ind == 'PRC', .N, by=xnmbr]


# merge disposition data with cases data
case_disp <- merge(cases, disp, by.x=c("xnmbr", "crt_case_nmbr"), by.y=c("xnmbr", "crt_case_nmbr"), all.x=TRUE,all.y=FALSE)

# select flash incarceration disposition cases within three years since the case grant date
# temp = cases + disposition
temp[year(case_grant_date) == 2012 & cnt_grant_typ_ind == "PRC" & dispo_description == "FLASH INCARCERATION" & dispo_date <= (case_grant_date %m+% months(36)), .N, by=.(xnmbr)]

case_disp$case_grant_date <- as.Date(as.POSIXlt(case_disp$case_grant_date, format="%m/%d/%Y"))

case_disp[year_grant == 2011 & cnt_grant_type_ind == "PRC" & dispo_description == "FLASH INCARCERATION" & dispo_date <= (case_grant_date %m+% months(36)) & dispo_date > case_grant_date,  .N, by=xnmbr]


dispo_date <= (case_grant_date %m+% months(36)) & dispo_date >= case_grant_date, 
