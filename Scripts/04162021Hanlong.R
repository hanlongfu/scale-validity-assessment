...
lapply(c("data.table", "lubridate", "zoo", "dplyr","readr", "Hmisc", "readr"), require, character.only=TRUE)
...
case_disp0[year_grant == 2011 & cnt_grant_type_ind == "PRC", .N, by=xnmbr]
...
case_disp0[year_grant == 2011 & cnt_grant_type_ind == "PRC" & dispo_desc %in% c("FLASH INCARCERATION", "ARRST WRRNT RECAL-FLASH INCARC") & dispo_date <= (case_grant_date %m+% months(36)),  .N, by=.(xnmbr)]
...
revoke_code <- c("PROBATION REVOKED - PRISON","	DIV/DEJ REVOKED - BWI","PROB REVOKED-PRISON STAYED","PROB REVOKED BWI-PROP36 TERM","PROBATION REVOKED - BWI","PROB REVOKED-PRISON 1731.5 WIC","PROB REVOKED-PRISON 1731.5 WIC","PROBATION REVOKED - JAIL","PROBATION REVOKED - IN CUSTODY")

# calculate the number of revocations in a mandatory supervision cohort
case_disp0[year_grant == 2011 & cnt_grant_type_ind == "MANDATORY SUPERVISION" & dispo_desc %in% revoke_code & dispo_date <= (case_grant_date %m+% months(36)),  .N, by=.(xnmbr)] 
...
# subset data for 2011 cohort
prc_cohort_2011 <- case_disp_assmnt[case_disp_assmnt$i.year_grant == 2011 & case_disp_assmnt$i.cnt_grant_type_ind== "PRC",]
setDT(prc_cohort_2011)[, idx := .N, by=.(i.xnmbr, i.crt_case_nmbr,x.assmnt_date)]
model0_fit <- glmer(formula = flash_inc ~ 1 + (1|i.xnmbr), data=prc_cohort_2011, family=binomial, nAGQ = 0, na.action=na.exclude)
summary(model0_fit)
...
model0_null <- glm(formula = flash_inc ~ 1, data = prc_cohort_2011, family=binomial, na.action=na.exclude)
...
model1_fit <- glmer(formula = flash_inc ~ 1 + assmnt_seq + (1|i.xnmbr), data=prc_cohort_2011, family=binomial, nAGQ = 0, na.action=na.exclude)
summary(model1_fit)
logLik(model0_fit) - logLik(model1_fit) # df=2

# use x.total_score to predict flash incarceration
# beta = 0.018, p <.001
model2_fit <- glmer(formula = flash_inc ~ 1 + x.total_score + (1|i.xnmbr), data=prc_cohort_2011, family=binomial, nAGQ = 0, na.action=na.exclude)
summary(model2_fit)
logLik(model0_fit) - logLik(model2_fit) # df=2
